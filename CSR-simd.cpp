...
int main() {
    const char *filename = "./data/1138_bus/1138_bus.mtx";
    double **matrix = NULL;
    CSRMatrix csrMatrix;
    int numRows, numCols;
...
int jacobiIterationCSR(const CSRMatrix *csrMatrix, const double *b, double *x, double tol, int maxIter) {
    ...
    double *x_new = (double *)_mm_malloc(numRows * sizeof(double), 4096);
    //double *x_new = (double *)aligned_alloc(4096, numRows * sizeof(double));
    for (int i = 0; i < numRows; i++) {
        x_new[i] = 0.0;  // 初始化新解向量
    }
    ...
    for (iter = 0; iter < maxIter; iter++) {
        ...
        // }
        int i = 0;
        for(; i <= numRows - 16; i += 16) {
            __m512 x_new_vals = _mm512_loadu_ps(&x_new[i]);
            __m512 x_vals = _mm512_loadu_ps(&x[i]);

            __m512 diff = _mm512_sub_ps(x_new_vals, x_vals);
            __m512 squared_diff = _mm512_mul_ps(diff, diff);

            norm += _mm512_reduce_add_ps(squared_diff);
        }
        for(; i < numRows; ++i) {
            norm += (x_new[i] - x[i]) * (x_new[i] - x[i]);
        }
